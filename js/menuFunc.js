	...
	function PopulateNewSelections()
	{
		...
		$("#addbtn").click(function(){
			...
			if(index<=newCircles.length) {
				  ...
				  $.post("PHP/insert_location_circular.php",{ name: newCircles[index-1].nameP, latitude: parseFloat(newCircles[index-1].latitude), longitude:parseFloat(newCircles[index-1].longitude), 
		   ...
		   alert(data);
			 ...
			 {
			 	 $.post("PHP/insert_location_rectangular.php",{ name: newRectangles[index-1-newCircles.length].nameP, 
...
function fetch_dataset(){
	$.ajax({
		 	url : 'PHP/fetch_location_circular.php',
		 	type : 'GET',
		 	async : true,
		...
		$.ajax({
		 	url : 'PHP/fetch_location_rectangular.php',
		 	type : 'GET',
		 	async : true,
		 	success : function(response) {
