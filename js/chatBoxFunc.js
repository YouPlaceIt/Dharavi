...
function get_message(){
	...
	$.post("PHP/insert_message.php",{ sender: user, message:currMsg, time: currTime, receiver:everybody});
...
function get_message_private(){
	...
	$.post("PHP/insert_message.php",{ sender: user, message:currMsg, time: currTime, receiver:receiver});
...
function chatBoxPopulate()
	{
		...
		clearOverlay();
		fetch_users();
		var a=document.getElementById("chatUser1");
		a.innerHTML= user
		remove();
		//fetch_users();
	...
	$.ajax({
		 	...
		 	success : function(response) {
		 		...
		 		if(allMessages.length>0)
		 			ch=allMessages[allMessages.length-1]['idM'];;
...
MessageTextBoxPriv='';
	...
	$.ajax({
		 	url : 'PHP/fetch_messages_private.php',
		 	type : 'GET',
		 	async : true,
